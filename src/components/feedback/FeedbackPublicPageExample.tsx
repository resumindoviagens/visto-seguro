...
export default function FeedbackPublicPageExample() {
  const [step, setStep] = useState<'auth' | 'code' | 'form' | 'done'>('auth');
      ...
      <section className="rounded-2xl border bg-white p-6 shadow-sm">
        ...
        {step === 'auth' && (
          <div className="mt-6 grid gap-3">
            ...
            <button className="rounded-xl bg-blue-900 p-3 text-white" onClick={() => setStep('code')}>
              Continuar
            </button>
          </div>
        )}
        ...
        {step === 'code' && (
          <div className="mt-6 grid gap-3">
            <p className="text-sm text-gray-600">Enviamos um código de 6 dígitos para seu email.</p>
            <input className="rounded-xl border p-3" placeholder="Código de verificação" />
        ...
        {step === 'done' && (
          <div className="mt-6 rounded-xl bg-green-50 p-4">
            ...
            <p className="text-sm text-gray-600">Sua resposta foi registrada com segurança.</p>
          </div>
        )}
      </section>
    </main>
