...
function addDefaultTime(dateOrDateTime, time = "09:00") {
  if (!dateOrDateTime) return "";
  const raw = String(dateOrDateTime);
  if (raw.includes("T")) return raw;
  return `${raw}T${time}:00-03:00`;
}
...
function eventKey(client, type, start) {
  return `${client.id}:${type}:${String(start || "").slice(0, 16)}`;
}
...
export function agendaEventsForClient(client) {
  ...
  if (isPassport(client)) {
    ...
    if (isRenewal(client)) {
      const start = addDefaultTime(client.casv_date || client.interview_date, "09:00");
      if (start) {
        events.push({
          type: "renovacao_casv",
      ...
      const start = addDefaultTime(client.interview_date || client.casv_date, "09:00");
      if (start) {
        events.push({
      ...
      if (client.casv_date) {
        const start = addDefaultTime(client.casv_date, "09:00");
        events.push({
          type: "casv",
      ...
      if (client.interview_date) {
        const start = addDefaultTime(client.interview_date, "09:00");
        events.push({
          type: "consulado",
...
async function alreadySent(clientId, event, emailType, recipient) {
  const key = eventKey({ id: clientId }, event.type, event.start);
...
export async function sendCasvVideoPlanningICS(client, { mode = "auto" } = {}) {
  ...
  await sendWithBrevo({
    ...
    html: simpleHtml("Marcar data de videochamada", [
      `Cliente: <strong>${client.name}</strong>`,
      `<strong>CASV:</strong> ${fmt(addDefaultTime(client.casv_date, "09:00"))}`,
...
export async function loadFutureClientsForAgenda() {
  ...
  const { data, error } = await supabaseAdmin
    .from("clients")
    .select("*")
    .or(`casv_date.gte.${today},interview_date.gte.${today},video_call_date.gte.${today},passport_pf_datetime.gte.${today}`);
