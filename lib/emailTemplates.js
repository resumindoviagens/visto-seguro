...
function layout({ title, body, ctaLabel, ctaUrl }) {
  const cta = ctaLabel && ctaUrl ? `
    <p style="margin:24px 0;text-align:center;">
      <a href="${ctaUrl}" style="background:#0f2a44;color:#ffffff;text-decoration:none;padding:14px 22px;border-radius:10px;display:inline-block;font-weight:700;">${ctaLabel}</a>
    </p>` : "";

  return `
  <div style="font-family:Arial,Helvetica,sans-serif;color:#1f2937;line-height:1.6;max-width:700px;margin:0 auto;background:#ffffff;padding:24px;border:1px solid #e5e7eb;border-radius:14px;">
    <div style="text-align:center;margin-bottom:18px;">
      <img src="${process.env.NEXT_PUBLIC_LOGO_URL || ""}" alt="Resumindo Viagens" style="max-width:190px;height:auto;display:${process.env.NEXT_PUBLIC_LOGO_URL ? "inline-block" : "none"};" />
    </div>
    <h2 style="color:#0f2a44;margin:0 0 16px;font-size:24px;line-height:1.25;">${title}</h2>
    ${body}
    ${cta}
    ...
    <p style="margin:0 0 14px;color:#4b5563;">Assessoria para solicitação de visto americano</p>
    ${goldenTrio()}
    <p style="margin:14px 0 0;color:#6b7280;font-size:12px;text-align:center;">Você recebeu este email porque iniciou atendimento com a Resumindo Viagens.</p>
  </div>`;
}
