...
export function getEmailTemplate(templateId, client, options = {}) {
  ...
  const videoCallDateTime = formatDateTimeBR(client?.video_call_date || client?.process_group?.video_call_date || options.videoCallDateTime || options.video_call_date);
    ...
    videochamada_agendada: {
      ...
      html: layout({
        ...
        body: `
          ...
          ${paragraph(`<strong>📅 Data e horário:</strong><br />${escapeHtml(videoCallDateTime || "[INSERIR DATA E HORÁRIO]")}`)}
