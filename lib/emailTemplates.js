...
function emailTheme(templateId) {
  const ny = ["formulario", "formulario_pendente", "formulario_recebido", "taxa_paga", "instrucoes"];
  return ny.includes(templateId) ? "nova-york" : "orlando";
}

function emailHeaderUrl(theme = "nova-york") {
  const base = process.env.NEXT_PUBLIC_SITE_URL || "https://app.resumindoviagens.com.br";
  const file = theme === "orlando" ? "header-orlando.png" : "header-nova-york.png";
  return `${base.replace(/\/$/, "")}/email-headers/${file}`;
}

function layout({ heading, greetingName, body, ctaLabel, ctaUrl, extraLinks, theme = "nova-york" }) {
  const header = emailHeaderUrl(theme);
  const themeLabel = theme === "orlando" ? "Orlando" : "Nova York";
  ...
  <div style="font-family:Arial,Helvetica,sans-serif;color:#1f2937;line-height:1.62;max-width:720px;margin:0 auto;background:#f6f8fb;padding:0;border:1px solid #e5e7eb;border-radius:18px;overflow:hidden;">
    <div style="background:#1f2a60;">
      <img src="${header}" alt="Resumindo Viagens - ${themeLabel}" width="720" style="width:100%;max-width:720px;height:auto;display:block;border:0;outline:none;text-decoration:none;" />
    </div>
    <div style="background:#ffffff;padding:28px;">
      <p style="margin:0 0 4px;color:#f59e0b;font-weight:700;font-size:14px;text-transform:uppercase;letter-spacing:.4px;">Resumindo Viagens • ${themeLabel}</p>
